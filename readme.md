...
1. **Многоуровневая валидация (LangGraph)**
   Мы реализовали не просто генерацию, а полноценный инженерный цикл:
...
4. **Защита от галлюцинаций (Уточняющие вопросы)**
   Если запрос неоднозначен или юзер просит доработать код, но не прикрепил его, системный промпт жестко блокирует "фантазию" модели. Агент возвращает уточняющий вопрос в виде валидного Lua-комментария (`-- Пожалуйста, уточните...`), не ломая парсеры проверяющей системы.
